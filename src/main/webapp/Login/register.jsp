...
<head>
    ...
    <title>Đăng ký – Thiết Bị Y Tế 24H</title>
...
<main class="auth-wrapper">
  <div class="auth-card">
    ...
    <div class="auth-card-header">
      ...
      <div class="auth-tabs">
        <button class="auth-tab" onclick="location.href='${pageContext.request.contextPath}/login'">Đăng nhập</button>
        <button class="auth-tab active" onclick="location.href='${pageContext.request.contextPath}/register'">Đăng ký</button>
      </div>
    </div>
    ...
    <div class="auth-card-body">
      ...
      <!-- Form đăng ký — giữ nguyên action, name fields -->
      <form id="formRegister" action="${pageContext.request.contextPath}/register" method="post" novalidate>
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input type="text" id="username" name="username"
                   placeholder="Nguyễn Văn A" autocomplete="username" required>
          </div>
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input type="email" id="email" name="email"
                   placeholder="nguyenvana@gmail.com" autocomplete="email" required>
          </div>
        ...
        <div class="form-group">
          ...
          <div class="input-wrapper">
            ...
            <input type="password" id="repassword" name="repassword"
                   placeholder="········" autocomplete="new-password"
                   class="has-toggle" oninput="checkMatch()" required>
            <button type="button" class="toggle-password" onclick="togglePwd('repassword', this)" aria-label="Hiện/ẩn mật khẩu">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="eye-icon">
                <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
              </svg>
            </button>
          ...
          <span class="field-error" id="err-repassword"></span>
        </div>
        ...
        <div class="form-group terms-row">
          <label class="checkbox-label">
            ...
            Tôi đồng ý với <a href="#" class="link-forgot">Điều khoản &amp; Chính sách bảo mật</a>
          </label>
...
function checkStrength(val) {
    ...
    if (score === 1) {
        ...
        label.classList.add('weak'); label.textContent = 'Yếu';
    } else if (score === 2) {
        bars[0].classList.add('medium'); bars[1].classList.add('medium');
        label.classList.add('medium'); label.textContent = 'Trung bình';
    } else {
        bars.forEach(b => b.classList.add('strong'));
        label.classList.add('strong'); label.textContent = 'Mạnh';
    }
}
...
function checkMatch() {
    ...
    if (!rpw) { ind.className = 'match-indicator'; inp.classList.remove('is-valid','is-invalid'); return; }
    if (pw === rpw) {
        ind.className = 'match-indicator show match'; ind.textContent = '✓ Mật khẩu khớp';
        inp.classList.add('is-valid'); inp.classList.remove('is-invalid');
    } else {
        ind.className = 'match-indicator show no-match'; ind.textContent = '✗ Mật khẩu không khớp';
...
document.getElementById('formRegister').addEventListener('submit', function(e) {
    ...
    [u, em, pw, rpw].forEach(i => i.classList.remove('is-invalid','is-valid'));
    ['err-username','err-email','err-password','err-repassword','err-terms']
    ...
    if (!em.value.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(em.value)) {
        document.getElementById('err-email').textContent = 'Vui lòng nhập email hợp lệ';
        em.classList.add('is-invalid'); valid = false;
    }
    ...
    if (!rpw.value || rpw.value !== pw.value) {
        document.getElementById('err-repassword').textContent = 'Mật khẩu xác nhận không khớp';
        rpw.classList.add('is-invalid'); valid = false;
    }
    if (!terms.checked) {
        document.getElementById('err-terms').textContent = 'Bạn cần đồng ý với điều khoản để tiếp tục';
        valid = false;
    }
